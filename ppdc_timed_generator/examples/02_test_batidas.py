...
from ppdc_timed_generator.generadores import GeneradorBatido
...
def test_crear_y_generar_en_N_minutos(poblacion: int, minutos: int):
    ...
    estacion1.generador = GeneradorBatido(
        estacion1.poblacion,
        hora_apertura=dt.time(7, 0),
        hora_cierre=dt.time(20, 0),
    )
    h = g_estado.hora_inicial
    estacion1.generador.current_datetime = dt.datetime(
        h.year, h.month, h.day, 7, 0
    )
